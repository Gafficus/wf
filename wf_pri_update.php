...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
			...
			$sql = "UPDATE wf_primary  set name = ?, type = ?, damage = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			...
			Database::disconnect();
			header("Location: wf_pri_list.php");
  }
} else { // if $_POST NOT filled then pre-populate the form
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT * FROM wf_primary where id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
  ...
  <body style="background-color: lightblue !important";>
    <div class="container">
      <div class="span10 offset1">
			...
			<form class="form-horizontal" action="wf_pri_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
        ...
        <div class="form-actions">
					...
					<a class="btn" href="wf_pri_list.php">Back</a>
				</div>
			</form>
