...
if ( !empty($_POST)) { // if not first time through
  ...
  //$idError = null;
	...
	if ($valid) {
		...
		$sql = "UPDATE wf_loadouts set per_id = ?, loadout_name = ?, frame_id = ?, primary_id = ?, secondary_id = ?, melee_id = ? where id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($wID, $wName,$wFrame,$wPrimary,$wSecondary,$wMelee, $id));
		Database::disconnect();
	...
	$pdo = Database::connect();
	$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$sql = "SELECT * FROM wf_loadouts where id = ?";
	$q = $pdo->prepare($sql);
	$q->execute(array($id));
	$data = $q->fetch(PDO::FETCH_ASSOC);
	$wID = $_SESSION['per_id'];
	$wName = $data['loadout_name'];
	$wFrame = $data['frame_id'];	
	$wPrimary = $data['primary_id'];		
	$wSecondary = $data['secondary_id'];	
	$wMelee = $data['melee_id'];	
	Database::disconnect();
...
<body style="background-color: lightblue !important";>
    <div class="container">
		<div class="span10 offset1">
			...
			<div class="row">
				<h3>Update Loadout</h3>
				<h3><?php echo $wName . "this"; ?></h3>
				
			</div>
	
			<form class="form-horizontal" action="wf_loa_update.php?id=<?php echo $id?>" method="post">						  
